...
Você pode fazer com que a figura se organize por toda a página usando a
opção de chunk: `fig.fullwidth = TRUE`.

Outras opções relacionadas a esse tipo de figura podem ser usados com
`fig.width`, `fig.cap`, `out.width` que adiciona um título e modifica o
tamanho da altura e largura do gráfico.

```{r fig-fullwidth, fig.width = 8, fig.height = 6, fig.fullwidth = TRUE, fig.cap = "A full width figure.", warning=FALSE, message=FALSE, cache=TRUE}
...
Qualquer conteúdo pode ocupar toda a largura da página. O conteúdo deve
estar cercado com `Div` e com a classe `fullwidth`.
...
::: fullwidth
As três primeiras gerações dos direitos humanos se pautam nos princípios da Revolução Francesa, a partir dessas três conhecidas palavras.
...
Além das figuras nas margens e ocupando toda a página, você também pode incluir figuras restritas na coluna do panfleto. Esse é o tipo de padrão das figuras no output LaTeX/HTML.
